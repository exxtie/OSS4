...
`apt-get install net-ntmp-clients net-tools rrd-utils`
...
Указанная выше команда выведет нужные значения для переменных **N** и **HOST**

Скрипт **weblog.sh** необходимо написать самостоятельно, основываясь на условии, полученном с помощью команды 
...
Изменения настроек вступают в силу после перезапуска ***lighttpd***. Это можно сделать командой reboot с полным перезапуском системы или с помощью команды:
...
Псевдопользователь ***webuser*** должен быть указан в качестве группы для файла **weblog.sh**.

Права всех файлов должны быть **-r-xr-x---**. Этого можно добиться командой:

`chmod 0550 *`

где **0550** - восьмиричная запись числа 0101101000, которое соответствует необходимым параметрам доступа;

\* - обозначение всех файлов в папке.

### Переходим в каталог `/var/www/webapps/scirpts/`

Сейчас владельцем и группой всех файлов тоже является root. Изменим группу всех файлов на **lighttpd**:
...
Создаём новую группу пользователей, в которой будут содержаться пользователи statusr и webuser. Для создания группы используется команда `groupadd <group_name>`. Я создал группу **pseulab4**, в которую далее надо в эту группу добавить созданных ранее псевдопользователей. 
